      ...
      if (taxExempt) {
        for (const folio of old.folios) {
          if (folio.status === 'OPEN') {
            // Retrieve TAX lines to create counter-entries
            const openTaxLines = await tx.folioLine.findMany({
              where: {
                folioId: folio.id,
                type: 'TAX',
                isAdjustment: false
            ...
            for (const taxLine of openTaxLines) {
              await tx.folioLine.create({
                data: {
                  folioId: folio.id,
                  type: 'TAX',
                  description: 'Annulation Rétroactive Taxe - Exemption Accordée',
                  unitPrice: -parseFloat(taxLine.amount.toString()),
                  quantity: 1,
                  amount: -parseFloat(taxLine.amount.toString()),
                  isAdjustment: true,
                  adjustedLineId: taxLine.id,
                  parentLineId: taxLine.parentLineId
                }
              });
            }
