...
export class UsersRolesService {
  ...
  async changePassword(userId: number, newPassword: string): Promise<void> {
    ...
    await this.prisma.user.update({
      ...
      data: {
        passwordHash,
        refreshTokenHash: null, // Invalidation de toutes les sessions actives
      },
    });

    await this.auditLogService.logAction(this.prisma, {
      userId,
      action: 'UPDATE',
      entityType: 'User',
      entityId: userId,
      newValues: { action: 'Password changed and sessions invalidated' },
    });
  }
}
