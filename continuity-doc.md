...
Le développement des phases 0 à 5 est validé et les modules suivants sont pleinement opérationnels :
...
*   **Phase 2 & 3 : Opérations Hôtelières**
    *   **Housekeeping :** Suivi des tâches, inspection et bascule automatique (Auto-clean). Seeding initial étendu avec des chambres de différents statuts.
...
*   **Phase 5 : Audit de Sécurité & Refonte RBAC**
    *   **Invalidation des tokens :** Invalidation explicite et systématique des *refresh tokens* en base de données lors d'un changement de mot de passe via l'interface profil, forçant la reconnexion sur tous les appareils.
    *   **Granularité des Permissions (Principe de Moindre Privilège) :** Séparation stricte entre les accès en lecture (`*.read`) et en écriture (`*.write`) dans les modules (Housekeeping, Maintenance, Guests, Billing). La navigation latérale (SidebarNav) s'adapte dynamiquement en ne vérifiant que les droits de lecture (`*.read`).

---

## 4. Cartographie de Sécurité Restante (Dette Technique)

Les vulnérabilités de sécurité identifiées ont été corrigées. Les chantiers d'amélioration continue pour les futures versions incluent :

*   **Rapports Avancés :** Finaliser le module d'exportation de données (PDF/CSV) pour les rapports de gestion.
*   **Tests Automatisés E2E :** Mettre en place des tests Cypress ou Playwright couvrant le flux complet d'une réservation (de la création à la clôture du folio).
