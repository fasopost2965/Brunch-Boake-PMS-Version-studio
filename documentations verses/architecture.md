...
### 7.4 Folio unique par séjour, clôture stricte et immuabilité
Un folio ne peut être clôturé (`close`) que si son solde est à zéro ou que le solde restant est explicitement acté comme "créance" avec justification. 
**Une fois clôturé, un folio et toutes ses lignes sont strictement immuables**.
Toute correction (ex. exemption de taxe rétroactive ou erreur de facturation) exige la création d'un **Folio d'Ajustement**. Ce dernier porte des écritures de contrepassation (montants négatifs) qui doivent être explicitement liées aux lignes d'origine via une clé `adjusted_line_id`. De même, les suppressions physiques (`DELETE`) de lignes de facturation sont proscrites (même sur folio ouvert pour des raisons de trace de caisse) ; on utilise l'annulation par contre-écriture.
