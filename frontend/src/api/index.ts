...
export const api = {
  ...
  rooms: {
    ...
    getById: async (id: string): Promise<Room | undefined> => {
      logApiCall('GET', `/api/rooms/${id} (fetch)`);
      try {
        const res = await fetch(`/api/rooms/${id}`);
        if (!res.ok) throw new Error('Échec HTTP ' + res.status);
        return await res.json();
      } catch (err) {
        console.warn(`[API CLIENT] Repli sur LocalStorage pour obtenir la chambre ${id}:`, err);
        return delay(roomsRepo.findOne(id));
      }
    },
    create: async (room: Room): Promise<Room> => {
      logApiCall('POST', '/api/rooms (fetch)', room);
      try {
        const res = await fetch('/api/rooms', {
          method: 'POST',
        ...
        if (!res.ok) {
          ...
          throw new Error(errData.error || 'Échec HTTP ' + res.status);
        }
        return await res.json();
      } catch (err: any) {
        console.warn('[API CLIENT] Repli sur LocalStorage pour insérer la chambre:', err);
        return delay(roomsRepo.insert(room));
      }
    },
    update: async (id: string, room: Partial<Room>): Promise<Room> => {
      logApiCall('PUT', `/api/rooms/${id} (fetch)`, room);
      try {
        const res = await fetch(`/api/rooms/${id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
        ...
        console.warn('[API CLIENT] Repli sur LocalStorage pour modifier la chambre:', err);
        return delay(roomsRepo.update(id, room));
      }
    },
    delete: async (id: string): Promise<boolean> => {
      logApiCall('DELETE', `/api/rooms/${id} (fetch)`);
      try {
        ...
        console.warn('[API CLIENT] Repli sur LocalStorage pour supprimer la chambre:', err);
  ...
  reservations: {
    getAll: async (): Promise<Reservation[]> => {
      logApiCall('GET', '/api/reservations (fetch)');
      try {
        const res = await fetch('/api/reservations');
        if (!res.ok) throw new Error('Échec HTTP ' + res.status);
        return await res.json();
      } catch (err) {
        console.warn('[API CLIENT] Connexion impossible au Backend MySQL. Repli sur LocalStorage pour les réservations:', err);
        return delay(reservationsRepo.get());
      }
    },
    getById: async (id: string): Promise<Reservation | undefined> => {
      logApiCall('GET', `/api/reservations/${id} (fetch)`);
      try {
        const res = await fetch(`/api/reservations/${id}`);
        if (!res.ok) throw new Error('Échec HTTP ' + res.status);
        return await res.json();
      } catch (err) {
        console.warn(`[API CLIENT] Repli sur LocalStorage pour obtenir la réservation ${id}:`, err);
        return delay(reservationsRepo.findOne(id));
      }
    },
    create: async (resData: Reservation): Promise<Reservation> => {
      logApiCall('POST', '/api/reservations (fetch)', resData);
      try {
        const res = await fetch('/api/reservations', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        console.warn('[API CLIENT] Repli sur LocalStorage pour créer la réservation:', err);
        return delay(reservationsRepo.insert(resData));
      }
    },
    update: async (id: string, resData: Partial<Reservation>): Promise<Reservation> => {
      logApiCall('PUT', `/api/reservations/${id} (fetch)`, resData);
      try {
        const res = await fetch(`/api/reservations/${id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
        ...
        console.warn('[API CLIENT] Repli sur LocalStorage pour modifier la réservation:', err);
        return delay(reservationsRepo.update(id, resData));
      }
    },
    delete: async (id: string): Promise<boolean> => {
      logApiCall('DELETE', `/api/reservations/${id} (fetch)`);
      try {
        ...
        console.warn('[API CLIENT] Repli sur LocalStorage pour supprimer la réservation:', err);
