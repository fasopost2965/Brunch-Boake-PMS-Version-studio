...
const ALL_PERMISSIONS: { code: string; description: string; module: string }[] = [
  // Users
  { code: 'users.read',                   description: 'Consulter les utilisateurs',                        module: 'users' },
  { code: 'users.create',                 description: 'Créer de nouveaux utilisateurs',                    module: 'users' },
  // Reservations
  { code: 'reservations.read',            description: 'Consulter les réservations',                        module: 'reservations' },
  ...
  // Guests
  { code: 'guests.read',                  description: 'Consulter les fiches clients',                      module: 'guests' },
  { code: 'guests.write',                 description: 'Créer et modifier des clients',                     module: 'guests' },
  // Settings / Rooms
  { code: 'settings.rooms.read',          description: 'Consulter les paramètres et types de chambres',     module: 'settings' },
  { code: 'settings.rooms.write',         description: 'Créer et modifier les types et chambres',           module: 'settings' },
  // Billing
  { code: 'billing.read',                description: 'Consulter les folios et factures',                  module: 'billing' },
  ...
  // Housekeeping
  { code: 'housekeeping.read',           description: 'Consulter les tâches de ménage',                    module: 'housekeeping' },
  { code: 'housekeeping.write',          description: 'Créer et mettre à jour les tâches de ménage',       module: 'housekeeping' },
  // Maintenance
  { code: 'maintenance.read',            description: 'Consulter les tickets de maintenance',              module: 'maintenance' },
...
const ROLE_PERMISSIONS: Record<string, string[]> = {
  Admin: [
    ...
    'reservations.read', 'reservations.create', 'reservations.write', 'reservations.checkin', 'reservations.checkout',
    'reservation.tax_exempt',
    'guests.read', 'guests.write',
    'settings.rooms.read', 'settings.rooms.write',
    'billing.read', 'billing.write', 'billing.close', 'billing.adjustment.create',
    'housekeeping.read', 'housekeeping.write',
    'maintenance.read', 'maintenance.write',
    'reports.read', 'reports.write',
  ],
  Manager: [
    // Manager: everything except user creation and system settings write
    'users.read',
    'reservations.read', 'reservations.create', 'reservations.write', 'reservations.checkin', 'reservations.checkout',
    'reservation.tax_exempt',
    'guests.read', 'guests.write',
  ...
  Receptionniste: [
    // Validated by product owner (2026-07-10) + Read updates:
    'reservations.read', 'reservations.create', 'reservations.write', 'reservations.checkin', 'reservations.checkout',
    'guests.read', 'guests.write',
...
async function main() {
  ...
  await prisma.room.upsert({
    where: { number: '101' },
    update: { occupancyStatus: 'OCCUPIED', cleanlinessStatus: 'CLEAN', technicalStatus: 'OPERATIONAL' },
    create: { number: '101', roomTypeId: roomType.id, floor: 1, occupancyStatus: 'OCCUPIED', cleanlinessStatus: 'CLEAN' },
  });

  await prisma.room.upsert({
    where: { number: '102' },
    update: { occupancyStatus: 'VACANT', cleanlinessStatus: 'DIRTY', technicalStatus: 'OPERATIONAL' },
    create: { number: '102', roomTypeId: roomType.id, floor: 1, occupancyStatus: 'VACANT', cleanlinessStatus: 'DIRTY' },
  });

  await prisma.room.upsert({
    where: { number: '103' },
    update: { occupancyStatus: 'VACANT', cleanlinessStatus: 'CLEAN', technicalStatus: 'OPERATIONAL' },
    create: { number: '103', roomTypeId: roomType.id, floor: 1, occupancyStatus: 'VACANT', cleanlinessStatus: 'CLEAN' },
  });

  await prisma.room.upsert({
    where: { number: '104' },
    update: { occupancyStatus: 'VACANT', cleanlinessStatus: 'INSPECTION', technicalStatus: 'OPERATIONAL' },
    create: { number: '104', roomTypeId: roomType.id, floor: 1, occupancyStatus: 'VACANT', cleanlinessStatus: 'INSPECTION' },
  });

  await prisma.room.upsert({
    where: { number: '105' },
    update: { occupancyStatus: 'VACANT', cleanlinessStatus: 'DIRTY', technicalStatus: 'MAINTENANCE' },
    create: { number: '105', roomTypeId: roomType.id, floor: 1, occupancyStatus: 'VACANT', cleanlinessStatus: 'DIRTY', technicalStatus: 'MAINTENANCE' },
  });
