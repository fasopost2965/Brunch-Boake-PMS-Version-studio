...
async function run() {
  ...
  if (linesOpen.length === 2 && parseFloat(linesOpen[1].amount as any) === -2000 && linesOpen[1].isAdjustment === true) {
    console.log('✅ TAX line on OPEN folio was counter-written (Adjustment).');
  } else {
    console.error('❌ TAX line on OPEN folio was NOT properly adjusted.');
  }
